...
To mitigate top-coding in brightly lit areas, the NGDC also released a radiance-calibrated series for eight years, which combines high magnification settings for low-light regions with low magnification settings for bright ones.
...
That assumption is restrictive, because regions differ in geography, in socio-economic conditions, and in policy implementation.
...
The vector $\boldsymbol{\alpha}$, with dimensions $k \times 1$, collects the associated regression coefficients.
...
The spatial weights matrix $\mathbf{W}$ determines which districts are treated as neighbors, so its specification is central to the analysis.
We employ a six-nearest-neighbors (6NN) matrix.
...
@ertur_koch_spatial_growth argues that the spatial Durbin model (SDM) accounts for such dependence in the convergence process.
