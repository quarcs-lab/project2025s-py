...
Regional growth and convergence matter most in large federal states such
as India, where spatial inequalities can threaten social cohesion and
political stability. Consistent economic data are rarely available below
the state level, and satellite nighttime lights have relaxed that
constraint as a proxy for economic activity. Using those data, Chanda
and Kabiraj (2020) document convergence across 520 Indian districts
between 1996 and 2010. Their approach, however, did not model spatial
dependence: neighboring luminosity enters only as a robustness control,
...
This article extends that analysis, and its contribution has three
dimensions. It is in part a replication and extension, adopting the
data, the sample, and the conditioning set of Chanda and Kabiraj (2020)
unchanged, so that any difference in the estimates reflects the spatial
specification alone. It is in part substantive: we build an interactive
visualization of convergence, test formally for spatial dependence in
both variables of the convergence equation, and estimate a spatial
Durbin model showing that catch-up across Indian districts has a
neighborhood dimension as well as a district one. The indirect effect of
initial luminosity is negative and statistically significant at the 10\%
level, so districts whose neighbors started from a lower base grew
faster than their own initial conditions alone would predict.

Its primary contribution, however, is methodological: a complete spatial
econometric study, from satellite imagery through the weight matrix and
the impact decomposition to the robustness checks, can be published so
that any reader can re-execute it in a browser. This journal has argued
for that form of publication (Rowe et al. 2020; M. Chen et al. 2020),
and both a Python notebook for nighttime lights (Patnaik and Mendez
2024) and a study of spatial inequality in Python and Quarto (Rey 2025)
have appeared here since. What this article adds is a complete applied
study rather than a tool or a demonstration dataset.

That the spatial specification raises rather than lowers the measured
speed is itself a substantive result. Convergence estimates are known to
respond to spatial dependence, but the direction of the response is not
settled: allowing for it lowers the estimated speed across European
regions and Indonesian districts, and reduces the estimated role of
initial income across Indian states (Isla-Castillo, Garashchuk, and
...
speed, from about 3.0\% to about 5.2\% per year, so the article adds a
signed estimate for a specific setting, benchmarked against the
comparable literature in the results section.

We also illustrate the wider potential of these data with an exploratory
analysis of luminosity and cultural participation across Indian states.
That exercise uses state-level data for an adjacent period and rank
correlations rather than the district-level convergence framework. We
present it as a demonstration of what the workflow makes possible, not
as evidence on convergence.

These findings carry implications for methodology and for policy:
non-spatial approaches understate the speed of convergence in settings
of this kind, because they omit the spatial structure of growth. For
policy, the implied half-life of regional disparities falls from roughly
23 years to 13, and if the indirect effects reflect genuine spillovers,
place-based policies would reach more districts than they target and be
more cost-effective than conventional estimates imply. The Indian
evidence also carries a warning, since a district-targeted tax program
raised activity partly by displacing it from neighbors that narrowly
missed qualifying (Hasan, Jiang, and Rafols 2021). Displaced activity
would appear in our framework as a spillover, so we weigh both readings
in the discussion.
...
five analytical notebooks are written in Python and run in Google
Colaboratory, so a reader needs no local installation and no licensed
software: clicking \emph{Run all} downloads the data and re-executes the
analysis. The sixth, N1, hosts the interactive visualization and its
Google Earth Engine source code. The notebooks, the data, and the
manuscript source are in the project repository at
...
\caption{Computational notebooks underlying this article. N2 to N6 open
in the browser through a Colab badge and run without local setup; N1
runs in the Earth Engine Code
Editor.}\label{tbl-notebooks}\tabularnewline
\toprule\noalign{}
...
Three constraints on what this approach can do yet stand out. The first
is the data, since the DMSP-OLS series is coarse, saturates in bright
urban cores, and blurs light across administrative boundaries. The
second is geography, since lights record only development that is
illuminated, so they are least informative where conventional statistics
are weakest. The third is theory, since luminosity is now applied well
beyond convergence analysis, yet it discriminates across places far
better than over time. We return to each in the discussion, where they
define a research agenda rather than a list of caveats.

The rest of this article is organized as follows. Section 2 describes
the data and methods and Section 3 the empirical results. Section 4
takes up the policy implications, the single growth period, the cultural
extension, and the three gaps above. Section 5 offers concluding
remarks.
...
from our spatial Durbin model indicate that the total convergence effect
in our fully specified model is approximately 51\% larger than
conventional non-spatial estimates, and that the implied half-life of
regional disparities falls from roughly 23 years to 13. This finding
suggests that in this setting non-spatial convergence models understate
the speed of regional convergence, although the comparison with other
regional literatures shows that the direction of this adjustment is
specific to the setting rather than general. If the estimated indirect
effects reflect genuine spillovers, place-based development
interventions would have broader impacts than conventional estimates
suggest, since their benefits would reach neighboring districts as well
as the districts they target. If instead they partly reflect activity
displaced across district boundaries, as the evaluation literature on
Indian place-based programs cautions, the same estimates would overstate
what those interventions add in aggregate.

The results also demonstrate the usefulness of satellite nighttime
lights for studying economic dynamics where subnational data are scarce,
infrequent, or unreliable. Radiance-calibrated luminosity allowed us to
analyze 520 Indian districts at a spatial granularity that conventional
national accounts could not reach. The approach is transferable to other
data-poor contexts across the developing world, and the ongoing
transition from DMSP-OLS to the higher-resolution VIIRS sensor should
sharpen such measurement further.

The analytical pipeline is documented in Jupyter notebooks written in
Python and embedded in the manuscript through the Quarto publishing
framework,\footnote{A single manuscript source generates multiple output
  formats. The HTML version allows readers to engage with interactive
  visualizations, inspect the code of the computational notebooks, and
  re-evaluate the results in light of their source code.} and hosting
the code and data in a public repository, with every notebook executable
in the cloud, is meant to encourage broader adoption of reproducible
open-science practices. Section 4 sets out where we believe the most
valuable work now lies: in better data, in the regions this method
currently serves least well, and in the questions beyond convergence
that it is able to address. The materials accompanying this article are
meant to make starting on any of them straightforward.
